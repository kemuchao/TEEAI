...
-(void) initDatasWithXMLElement:(GDataXMLElement *) xmlElement
{
    ...
    for (int i = 0; i < [childrenElements count]; i++) {
        ...
        if ([[ele name] isEqualToString:@"ssid"]) {
            self.ssid = ele.stringValue;
        } else if ([[ele name] isEqualToString:@"enc"]) {
            self.enc = ele.stringValue;
        } else if ([[ele name] isEqualToString:@"cipher"]) {
            self.cipher = ele.stringValue;
        } else if ([[ele name] isEqualToString:@"signal"]) {
            self.signal = ele.stringValue;
        }
    }
}
@end
