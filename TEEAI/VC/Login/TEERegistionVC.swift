...
class TEERegistionVC: ABCBaseVC {

    @IBOutlet weak var codeButton: UIButton!
    
    @IBOutlet weak var enterPasTextField: UITextField!
    ...
    @IBAction func getCode(_ sender: Any) {
        ...
        ABCProgressHUD.show()
        // 发送验证码
        user.sendCode(phone: validString(phoneTextField.text), type: 1) { (data, error) in
...
//        self.performSegue(withIdentifier: SegueIdentifier.toCommitMessage.rawValue, sender: self)
        ...
        guard validString(pasTextField.text) != "" else {
            ABCProgressHUD.showErrorWithStatus(string: "请输入密码")
            return
        }
        guard validString(enterPasTextField.text) != "" else {
            ABCProgressHUD.showErrorWithStatus(string: "请输入确认密码")
            return
        }
        guard validString(enterPasTextField.text) == validString(pasTextField.text) else {
            ABCProgressHUD.showErrorWithStatus(string: "两次密码输入不一样")
            return
        }
        phone = validString(phoneTextField.text)
        ABCProgressHUD.show()
        user.register(phone: phone, verify: validString(codeTextField.text), password: validString(enterPasTextField.text)) { (data, error) in
            ABCProgressHUD.dismiss()
            if error == nil {
                ...
                printX(error)
                let dict = validDictionary(error)
                if validInt(dict["code"]) == APICodeType.UserAlreadyExist.rawValue {
                    ABCProgressHUD.showErrorWithStatus(string: "该手机号码已被注册")
                }else if validInt(dict["code"]) == APICodeType.CodeRepeat.rawValue {
                    ABCProgressHUD.showErrorWithStatus(string: "验证码错误")
                }else {
                    ABCProgressHUD.showErrorWithStatus(string: "注册失败")
//                    ABCProgressHUD.showSuccessWithStatus(string: "注册成功")
                }
            }
        }
...
// MARK: - 跳转
extension TEERegistionVC: SegueHandlerType {
